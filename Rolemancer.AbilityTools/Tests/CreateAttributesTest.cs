...
{
    public class CreateAttributesTest : MonoBehaviour
    {
        private void Start()
        {
            TestAttributes();
        ...
        {
            var job = new PrintJob() {TargetId = targetId, Effects =  Mapping.Map.Attributes};
            var x = job.Schedule();
            x.Complete();
        }
        ...
        {
            public TargetAttributeCollection Effects;
            public TargetId TargetId;
            public void Execute()
            {
                var attributes = Effects.GetTargetAttributes(TargetId, Allocator.Temp);//TargetId.GetAttributes(Allocator.Temp);
